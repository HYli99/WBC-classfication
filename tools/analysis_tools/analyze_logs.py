...
def plot_phase_train(metric, log_dict, epochs, curve_label, json_log):
    ...
    # Subsample the data: take every 2nd point
    xs_subsampled = xs[::30]
    ys_subsampled = ys[::30]

    plt.xlabel('Epochs')
    #plt.plot(xs, ys, label=curve_label, linewidth=0.75)
    plt.plot(xs_subsampled, ys_subsampled, label=curve_label, linewidth=0.75)
...
def plot_curve_helper(log_dicts, metrics, args, legend):
    ...
    # 设置X轴、Y轴标签和标题的字体和大小
    plt.xlabel('X Axis Label', fontproperties=font, fontweight='bold')
    #plt.ylabel('Y Axis Label', fontproperties=font, fontweight='bold')
    #plt.title('Chart Title', fontproperties=font, fontsize=16)

    #设置横纵坐标刻度的字体和大小
    plt.tick_params(axis='both', which='major', labelsize=14, fontproperties=font)
